...
export class Draw {
    ...
    public insertElementList(
        ...
        console.log('insertElementList被调用', {payload, options});
        if (!payload.length || !this.range.getIsCanInput()) return
        const {startIndex, endIndex} = this.range.getRange()
        if (!~startIndex && !~endIndex) return
        const {isSubmitHistory = true, ensureWrapInParagraph = true} = options
        console.log('ensureWrapInParagraph选项值:', ensureWrapInParagraph);

        // 确保文本元素总是被包装在段落中
        let processedPayload;
        if (ensureWrapInParagraph) {
            console.log('准备调用ensureTextInParagraph');
            processedPayload = ensureTextInParagraph(payload);
            console.log('ensureTextInParagraph处理后:', processedPayload);
        } else {
            processedPayload = payload;
        }

        formatElementList(processedPayload, {
        ...
        if (activeControl && this.control.getIsRangeWithinControl()) {
            curIndex = activeControl.setValue(processedPayload, undefined, {
            ...
            this.spliceElementList(elementList, start, 0, processedPayload)
            curIndex = startIndex + processedPayload.length
            // 列表前如有换行符则删除-因为列表内已存在
            const preElement = elementList[start - 1]
            if (
                processedPayload[0].listId &&
                preElement &&
                !preElement.listId &&
    ...
    public appendElementList(
        ...
        console.log('appendElementList被调用', {elementList, options});
        if (!elementList.length) return

        // 确保文本元素总是被包装在段落中
        const {isPrepend, isSubmitHistory = true, ensureWrapInParagraph = true} = options
        console.log('ensureWrapInParagraph选项值:', ensureWrapInParagraph);

        let processedElementList;
        if (ensureWrapInParagraph) {
            console.log('准备调用ensureTextInParagraph');
            processedElementList = ensureTextInParagraph(elementList);
            console.log('ensureTextInParagraph处理后:', processedElementList);
        } else {
            processedElementList = elementList;
        }

        formatElementList(processedElementList, {
            isHandleFirstElement: false,
            editorOptions: this.options
        })
        let curIndex: number
        if (isPrepend) {
            this.elementList.splice(1, 0, ...processedElementList)
            curIndex = processedElementList.length
        } else {
            this.elementList.push(...processedElementList)
    ...
    /**
     * 专门用于处理粘贴内容的插入
     * 与普通insertElementList的主要区别是避免添加额外的零宽空格和换行符
     */
    public insertPastedElementList(
        payload: IElement[],
        options: IInsertElementListOption = {}
    ) {
        console.log('insertElementList被调用', {payload, options});
        ...
        // 处理粘贴内容中的段落ID，确保每个段落有唯一ID
        let currentParagraphId = '';
        let isFirstParagraph = true; // 标记是否是第一个段落

        // 检查目标位置的段落情况
        const elementList = this.getElementList();
        let targetHasContentInParagraph = false;

        if (~startIndex) {
                        // 查找光标所在的段落
            const cursorElement = elementList[startIndex];
            console.log('cursorElement详细:', {
                cursorElement,
                startIndex,
            ...
            if (cursorElement && cursorElement.id) {
                ...
                if (paragraphHasContent) {
                    targetHasContentInParagraph = true;
                  console.log('cursorElement:',[cursorElement])
                    console.log('目标段落有内容，第一个段落将使用目标段落ID');
                }
            }
        }

        for (let i = 0; i < processedPayload.length; i++) {
            const element = processedPayload[i];
            ...
            if (element.value === ZERO || element.value === '\n') {
                // 只有当不是段落元素内部的换行时才重置ID
                if (i === 0 || i === processedPayload.length - 1 ||
                    processedPayload[i-1].type !== ElementType.PARAGRAPH) {
                    currentParagraphId = '';
                }
                continue;
            }
    ...
    public setValue(payload: Partial<IEditorData>, options?: ISetValueOption) {
        ...
        const curIndex = isSetCursor
            ? processedMain?.length
                ? processedMain.length - 1
                : 0
            : undefined
    ...
    public render(payload?: IDrawOption) {
        ...
        nextTick(() => {
            ...
            setTimeout(() => {
                console.log('渲染完成，检查DOM元素:');
                const paragraphElements = document.querySelectorAll('.canvas-editor-paragraph');
                if (paragraphElements.length > 0) {
                    paragraphElements.forEach((el, index) => {
                        const computedStyle = window.getComputedStyle(el);
                        console.log(`段落${index}样式:`, {
                ...
                // else {
                //   console.log('未找到段落元素，可能使用了Canvas渲染');
                //
                //
                // }
            }, 100);
