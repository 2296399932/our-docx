...
def compare_styles(doc_path, classification_path, style_mapping_path, api_params_path):
        ...
        for element_class, style_class in section_mapping.items():
            ...
            for para_idx in para_indices:
                try:
                    ...
                    para_element = doc.elements[para_idx]['element'] if isinstance(doc.elements[para_idx], dict) and 'element' in doc.elements[para_idx] else doc.elements[para_idx]
                    ...
                    # 判断段落样式是否完全匹配
                    para_match = all(matched for attr, matched in para_results.items() if attr != 'results') if para_results else False
                    if para_match:
                    ...
                    if run_count > 0:
                        ...
                        for run_idx in range(run_count):
                            run_text = doc.get_run_text(para_idx, run_idx)
                            if run_text.strip():
                        ...
                        for run_idx in range(run_count):
                            ...
                            run_complete_style_info = style_analyzer.get_run_complete_style_info(para_element, run_element, run_idx)
                            ...
                            # 判断run样式是否完全匹配
                            run_match = all(matched for attr, matched in run_results.items() if attr != 'results') if run_results else False
                            if run_match:
    ...
    print(f"段落: {statistics['paragraphs_matching']}/{statistics['paragraphs']} 匹配 ({statistics['paragraphs_matching']/statistics['paragraphs']*100:.1f}% 如果有段落)")

    if statistics['runs'] > 0:
        print(f"非空Runs: {statistics['runs_matching']}/{statistics['runs']} 匹配 ({statistics['runs_matching']/statistics['runs']*100:.1f}%)")
...
def compare_run_style(run_effective_style, target_style, prefix=""):
        ...
        if 'eastAsia' in fonts:
            ...
            if match_indicator == "✗":
                match_results['results'].append({
                    'font_chinese': {'scuccess': target_font,'error': actual_font}})
            ...
            match_results['results'].append({
                'font_chinese': {'scuccess': target_style['font_chinese']}})
    ...
    if 'font_ascii' in target_style:
        if 'ascii' in fonts:
            ...
            if match_indicator == "✗":
                match_results['results'].append({
                    'font_ascii': {'scuccess': target_font,'error': actual_font}})
        else:
            print(f"{prefix}! font_ascii: 目标值={target_style['font_ascii']}, 实际值=未定义")
            match_results['font_ascii'] = False
            match_results['results'].append({
                'font_ascii': {'scuccess': target_style['font_ascii']}})
    ...
    if 'size' in target_style:
        if 'size' in run_properties:
            ...
            try:
                ...
                if (target_size == 24 and actual_size == 21) or (target_size == 21 and actual_size == 24):
                    matches = True
                    print(f"{prefix}✓ size: 目标值={target_size}, 实际值={actual_size} (五号与小四号视为兼容)")
                # 其他情况使用标准误差范围
                elif abs(actual_size - target_size) <= 2:
                    matches = True
                    match_indicator = "✓"
                    ...
                    print(f"{prefix}{match_indicator} size: 目标值={target_size}, 实际值={actual_size}")
                    match_results['results'].append({
                        'size': {'scuccess': target_size,'error': actual_size}})
                ...
                match_results['results'].append({
                   'size': {'scuccess': target_size}
                })
        else:
            print(f"{prefix}! size: 目标值={target_style['size']}, 实际值=未定义")
            match_results['size'] = False
            match_results['results'].append({
                  'size': {'scuccess': target_style['size']}
            })
    ...
    if 'bold' in target_style:
        if 'bold' in run_properties:
            ...
            if match_indicator == "✗":
                match_results['results'].append({
                    'bold': {'scuccess': target_bold,'error': actual_bold}})
        else:
...
def compare_paragraph_style(actual_style, target_style, prefix=""):
    ...
    if 'alignment' in target_style:
        if 'paragraph_properties' in actual_style and 'alignment' in actual_style['paragraph_properties']:
            ...
            if (target_alignment == 'left' and actual_alignment == 'both') or (target_alignment == 'both' and actual_alignment == 'left'):
                matches = True
                ...
                if match_indicator == "✗":
                    match_results['results'].append({'alignment':{'scuccess':target_alignment,'error':actual_alignment}})

            match_results['alignment'] = matches
        else:
            # 实际样式中不存在alignment属性
            print(f"{prefix}! alignment: 目标值={target_style['alignment']}, 实际值=未定义")
            match_results['results'].append({
                'alignment': { 'scuccess': target_style['alignment']}})
            match_results['alignment'] = False

    # 2. 比较缩进 (first_line, hanging)
    if 'first_line' in target_style:
        if 'paragraph_properties' in actual_style and 'indentation' in actual_style['paragraph_properties'] and 'firstLine' in actual_style['paragraph_properties']['indentation']:
            ...
            if actual_first_line is not None:
                ...
                try:
                    ...
                    # 例如：如果目标是2个字符宽度(880)，实际值是1个字符宽度(440)，视为接近
                    if abs(actual_first_line - target_first_line) <= 440:
                        matches = True
                        print(f"{prefix}✓ first_line: 目标值={target_first_line}, 实际值={actual_first_line} (允许较大误差)")
                    else:
                        matches = False
                        match_indicator = "✗"
                        print(f"{prefix}{match_indicator} first_line: 目标值={target_first_line}, 实际值={actual_first_line}")
                        match_results['results'].append({
                            'first_line': {'scuccess': target_first_line, 'error': actual_first_line}})
                    ...
                    match_results['results'].append({
                        'first_line': {'scuccess': target_first_line}})
        else:
            print(f"{prefix}! first_line: 目标值={target_style['first_line']}, 实际值=未定义")
            match_results['first_line'] = False
            match_results['results'].append({
                'first_line': {'scuccess': target_style['first_line']}})

    if 'hanging' in target_style:
        if 'paragraph_properties' in actual_style and 'indentation' in actual_style['paragraph_properties'] and 'hanging' in actual_style['paragraph_properties']['indentation']:
            ...
            if actual_hanging is not None:
                try:
                    ...
                    if match_indicator == "✗":
                        match_results['results'].append({
                            'hanging': {'scuccess': target_hanging, 'error': actual_hanging}})
                except:
                    match_results['hanging'] = False
                    print(f"{prefix}? hanging: 目标值={target_hanging}, 实际值={actual_hanging} (无法比较)")
                    match_results['results'].append({
                        'hanging': {'scuccess': target_hanging}})
        else:
            print(f"{prefix}! hanging: 目标值={target_style['hanging']}, 实际值=未定义")
            match_results['hanging'] = False
            match_results['results'].append({
                'hanging': {'scuccess': target_style['hanging']}})

    # 3. 比较行距和段落间距
    if 'line' in target_style:
        if 'paragraph_properties' in actual_style and 'spacing' in actual_style['paragraph_properties'] and 'line' in actual_style['paragraph_properties']['spacing']:
                ...
                try:
                    ...
                    if target_line_rule and actual_line_rule and target_line_rule != actual_line_rule:
                        ...
                        if (target_line_rule == 'exact' and actual_line_rule == 'auto' and
                            ...
                            matches = True
                            print(f"{prefix}✓ line: 目标值={target_line}({target_line_rule}), 实际值={actual_line}({actual_line_rule}) (不同规则但等效)")
                        else:
                            matches = False
                            match_indicator = "✗"
                            print(f"{prefix}{match_indicator} line: 目标值={target_line}({target_line_rule}), 实际值={actual_line}({actual_line_rule})")
                            match_results['results'].append({
                                'line': {'scuccess': target_line, 'error': actual_line}})
                    else:
                        # 相同规则下的常规比较
                    ...
                    match_results['results'].append({
                        'line': {'scuccess': target_line}})
        else:
            print(f"{prefix}! line: 目标值={target_style['line']}, 实际值=未定义")
            match_results['line'] = False
            match_results['results'].append({
                'line': {'scuccess': target_style['line']}})

    if 'line_rule' in target_style:
        if 'paragraph_properties' in actual_style and 'spacing' in actual_style['paragraph_properties'] and 'lineRule' in actual_style['paragraph_properties']['spacing']:
            ...
            if actual_line_rule is not None:
                ...
                if ((target_line_rule == 'exact' and actual_line_rule == 'auto') or
                    ...
                    if abs(int(target_line) - int(actual_line)) < 120:  # 120 twips (6 points) 的误差容忍度
                        matches = True
                        print(f"{prefix}✓ line_rule: 目标值={target_line_rule}, 实际值={actual_line_rule} (考虑行距值后视为等效)")
                    else:
                        matches = False
                        match_indicator = "✗"
                        print(f"{prefix}{match_indicator} line_rule: 目标值={target_line_rule}, 实际值={actual_line_rule}")
                        match_results['results'].append({
                            'line_rule': {'scuccess': target_line_rule, 'error': actual_line_rule}})
                else:
                    # 常规比较
            ...
            match_results['results'].append({
                'line_rule': {'scuccess': target_style['line_rule']}})
            match_results['line_rule'] = False

    if 'before' in target_style:
        if 'paragraph_properties' in actual_style and 'spacing' in actual_style['paragraph_properties'] and 'before' in actual_style['paragraph_properties']['spacing']:
    ...
    if 'after' in target_style:
        if 'paragraph_properties' in actual_style and 'spacing' in actual_style['paragraph_properties'] and 'after' in actual_style['paragraph_properties']['spacing']:
        ...
        for attr in ['font_chinese', 'font_ascii', 'size', 'bold']:
            if attr in target_style:
                if attr == 'bold':
                    ...
                    if target_bold == False:
                        matches = True
                        print(f"{prefix}✓ bold: 目标值={target_bold}, 实际值={actual_bold} (默认为False，无run_properties)")
                    else:
                        matches = False
                        print(f"{prefix}✗ bold: 目标值={target_bold}, 实际值={actual_bold} (默认为False，无run_properties)")
                        match_results['results'].append({
                    ...
                    match_results['results'].append({
                        attr: {'scuccess': target_style[attr]}
                    })
...
def compare_tables_style(doc, table_styles, statistics):
    ...
    for i in range(table_count):
        ...
        if 'results' in match_results:
            ...
            table_element = current_table['element'] if isinstance(current_table, dict) and 'element' in current_table else current_table
    ...
    print(f"完全匹配的表格: {table_matches['matching']}/{table_count} ({table_matches['matching']/table_count*100:.1f}%)")
    print(f"列名行匹配的表格: {table_matches['header_row_matching']}/{table_count} ({table_matches['header_row_matching']/table_count*100:.1f}%)")
    print(f"数据行匹配的表格: {table_matches['data_row_matching']}/{table_count} ({table_matches['data_row_matching']/table_count*100:.1f}%)")
    print(f"三线表状态匹配的表格: {table_matches['three_line_matching']}/{table_count} ({table_matches['three_line_matching']/table_count*100:.1f}%)")
...
def extract_table_row_styles(doc, table_index):
        ...
        try:
            ...
            if len(rows) > 0:
                ...
                if first_row_cells:
                    ...
                    # 检查表头顶部边框
                    top_border = first_cell.find(f".//{{{NAMESPACES['w']}}}tcPr/{{{NAMESPACES['w']}}}tcBorders/{{{NAMESPACES['w']}}}top", NAMESPACES)
                    if top_border is not None:
                    ...
                    # 检查表头底部边框
                    bottom_border = first_cell.find(f".//{{{NAMESPACES['w']}}}tcPr/{{{NAMESPACES['w']}}}tcBorders/{{{NAMESPACES['w']}}}bottom", NAMESPACES)
                    if bottom_border is not None:
            ...
            if len(rows) > 0:
                ...
                if result["borders"]["table_bottom"] is None and last_row_cells:
                    ...
                    cell_bottom = last_cell.find(f".//{{{NAMESPACES['w']}}}tcPr/{{{NAMESPACES['w']}}}tcBorders/{{{NAMESPACES['w']}}}bottom", NAMESPACES)
    ...
    if "is_three_line_table" in extracted_styles and "is_three_line_table" in target_styles:
        ...
        print(f"{prefix}{match_indicator} 三线表: 目标值={target_styles['is_three_line_table']}, 实际值={extracted_styles['is_three_line_table']}")
        if match_indicator=="✗":
            match_results['results'].append({'三线表': {'scuccess': target_styles['is_three_line_table'], 'error': extracted_styles['is_three_line_table']}})
    ...
    for key in ["font_chinese", "font_ascii", "size", "bold", "alignment", "line", "line_rule"]:
        if key in target_styles["列名行文本样式"]:
            ...
            if key in ["font_chinese", "font_ascii"]:
                ...
                if match_indicator == "✗":
                    match_results['results'].append({'column_header': {'attribute': key, 'scuccess': target_value, 'error': extracted_value}})
    ...
    for key in ["font_chinese", "font_ascii", "size", "bold", "alignment", "line", "line_rule"]:
        if key in target_styles["数据行文本样式"]:
            ...
            if key in ["font_chinese", "font_ascii"]:
                ...
                print(f"{prefix}  {match_indicator} {key}: 目标值={target_value}, 实际值={extracted_value}")
                if match_indicator == "✗":
                    match_results['results'].append({'data_row': {'attribute': key, 'scuccess': target_value, 'error': extracted_value}})
            elif key == "size":
                try:
                    # 特殊处理字号对应关系
                    if  abs(extracted_value - target_value) <= 1:
...
if __name__ == "__main__":
    ...
    if missing_files:
        ...
        # 执行样式比较
        compare_styles(doc_path, classification_path, style_mapping_path, api_params_path)