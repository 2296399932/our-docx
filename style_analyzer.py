...
class StyleAnalyzer(DocxElementParser):
        ...
        for key, value in direct_style.items():
            if key not in ['style_id'] and value:  # 跳过空值
                ...
                if key == 'alignment' and 'paragraph_properties' in effective_style:
                    ...
                    if not effective_style['paragraph_properties'].get('indentation'):
                        effective_style['paragraph_properties']['indentation'] = {}
                    effective_style['paragraph_properties']['indentation'].update(value)
                elif key == 'spacing' and 'paragraph_properties' in effective_style:
                    if not effective_style['paragraph_properties'].get('spacing'):
                        effective_style['paragraph_properties']['spacing'] = {}
                    effective_style['paragraph_properties']['spacing'].update(value)
    ...
    def print_style_hierarchy(self):
        ...
        print("\n=== 样式继承层级 ===")
        # 找到顶级样式 (没有父级的样式)
        top_styles = set(self.style_map.keys()) - set(style for styles in self.style_hierarchy.values() for style in styles)
    ...
    def get_run_complete_style_info(self, para_element, run_element, run_index=None):
        ...
        if default_character_style and 'run_properties' in default_character_style:
            import copy
            effective_style['run_properties'] = copy.deepcopy(default_character_style['run_properties'])

        # 6.2 然后应用段落的有效样式中与run相关的部分
        if 'run_properties' in paragraph_effective_style:
            if 'run_properties' not in effective_style:
                effective_style['run_properties'] = {}
            ...
            for subkey, subvalue in paragraph_effective_style['run_properties'].items():
                if isinstance(subvalue, dict) and subkey in effective_style['run_properties']:
                    # 对嵌套字典进行深度合并
                    effective_style['run_properties'][subkey] = {**effective_style['run_properties'][subkey], **subvalue}
                else:
                    effective_style['run_properties'][subkey] = subvalue

        # 6.3 然后应用run的模板样式 (如果有)
        if template_style and style_id in self.effective_styles:
            template_effective = self.effective_styles[style_id]
            if 'run_properties' in template_effective:
                ...
                for subkey, subvalue in template_effective['run_properties'].items():
                    if isinstance(subvalue, dict) and subkey in effective_style['run_properties']:
                        # 对于嵌套字典(如fonts)，需要完全替换而不是合并
                        effective_style['run_properties'][subkey] = copy.deepcopy(subvalue)
                    else:
                        effective_style['run_properties'][subkey] = subvalue

        # 6.4 最后应用run的直接样式 (这部分应该完全覆盖之前的设置)
        if 'run_properties' not in effective_style:
            effective_style['run_properties'] = {}

        # 直接从run的XML元素提取样式，确保覆盖所有继承的样式
        # 这样比调用_map_run_style_to_properties更可靠，因为我们直接从XML获取属性
        if rPr is not None:
            # 处理字体属性 - 注意这里要完全替换而不是合并
            fonts_elem = rPr.find(".//w:rFonts", self.NAMESPACES)
            if fonts_elem is not None:
                # 如果run直接指定了任何字体，我们需要创建一个新的fonts字典
                if 'fonts' not in effective_style['run_properties'] or not effective_style['run_properties']['fonts']:
                ...
                for attr in ['ascii', 'hAnsi', 'eastAsia', 'cs']:
                    ...
                    if val is not None:
                        # 直接设置会覆盖继承的值
                        effective_style['run_properties']['fonts'][attr] = val
            ...
            if szCs_elem is not None:
                effective_style['run_properties']['sizeCs'] = szCs_elem.get(f"{{{self.NAMESPACES['w']}}}val")

            # 处理加粗
            bold = rPr.find(".//w:b", self.NAMESPACES)
            if bold is not None:
                effective_style['run_properties']['bold'] = bold.get(f"{{{self.NAMESPACES['w']}}}val", "true")

            # 处理斜体
            italic = rPr.find(".//w:i", self.NAMESPACES)
            if italic is not None:
                effective_style['run_properties']['italic'] = italic.get(f"{{{self.NAMESPACES['w']}}}val", "true")

            # 处理颜色
            color = rPr.find(".//w:color", self.NAMESPACES)
            if color is not None:
                effective_style['run_properties']['color'] = color.get(f"{{{self.NAMESPACES['w']}}}val")

            # 处理下划线
            underline = rPr.find(".//w:u", self.NAMESPACES)
            if underline is not None:
                effective_style['run_properties']['underline'] = underline.get(f"{{{self.NAMESPACES['w']}}}val", "single")

            # 处理突出显示
            highlight = rPr.find(".//w:highlight", self.NAMESPACES)
            if highlight is not None:
                effective_style['run_properties']['highlight'] = highlight.get(f"{{{self.NAMESPACES['w']}}}val")

            # 处理垂直对齐(上标/下标)
            vert_align = rPr.find(".//w:vertAlign", self.NAMESPACES)
            if vert_align is not None:
                effective_style['run_properties']['vertAlign'] = vert_align.get(f"{{{self.NAMESPACES['w']}}}val")

            # 处理删除线
            strike = rPr.find(".//w:strike", self.NAMESPACES)
            if strike is not None:
                effective_style['run_properties']['strike'] = strike.get(f"{{{self.NAMESPACES['w']}}}val", "true")

            # 处理双删除线
            dstrike = rPr.find(".//w:dstrike", self.NAMESPACES)
            if dstrike is not None:
                effective_style['run_properties']['dstrike'] = dstrike.get(f"{{{self.NAMESPACES['w']}}}val", "true")

            # 处理大小缩放
            w_scale = rPr.find(".//w:w", self.NAMESPACES)
            if w_scale is not None:
                effective_style['run_properties']['w_scale'] = w_scale.get(f"{{{self.NAMESPACES['w']}}}val")

            # 处理字符间距
            spacing = rPr.find(".//w:spacing", self.NAMESPACES)
            if spacing is not None:
                effective_style['run_properties']['char_spacing'] = spacing.get(f"{{{self.NAMESPACES['w']}}}val")
