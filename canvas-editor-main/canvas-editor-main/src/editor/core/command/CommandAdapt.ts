...
export class CommandAdapt {
    ...
    public indent() {
        console.log('indent方法被调用');

        // 1. 优先获取选中段落的 element 列表

        const elementList = this.range.getRangeParagraphElementList() || [];
        console.log('选中的段落列表:', elementList);
        ...
        if (elementList.length) {
            ...
            elementList.forEach(el => {
                ...
                el.indent = Math.min(currentIndent + 1, 100);
                console.log('设置缩进:', el.indent, el);
            });
        ...
        const el = mainList[endIndex];
        if (el) {
            const currentIndent = el.indent || 0;
            el.indent = Math.min(currentIndent + 1, 100);
            console.log('设置缩进(光标):', el.indent, el);
            // 强制重新计算和渲染
            this.draw.render({
                isCompute: true,
                isSetCursor: true,
    ...
    public outdent() {
        ...
        if (elementList.length) {
            ...
            elementList.forEach(el => {
                // 确保缩进值不会小于0
                const currentIndent = el.indent || 0;
                el.indent = Math.max(currentIndent - 1, 0);
                console.log('减少缩进:', el.indent, el);
            });
        ...
        if (el) {
            const currentIndent = el.indent || 0;
            el.indent = Math.max(currentIndent - 1, 0);
            console.log('减少缩进(光标):', el.indent, el);
            // 强制重新计算和渲染
            this.draw.render({
                isCompute: true,
                isSetCursor: true,
    ...
    /**
     * 直接设置选中段落的缩进值
     * @param value 要设置的缩进值(0-1000)
     */
    public setIndentValue(value: number) {
        const {startIndex, endIndex} = this.range.getRange();
        // 限制缩进值在0-1000之间
        const targetIndent = Math.max(0, Math.min(value, 1000));
        try {
            // 获取选区内的元素列表
            const rangeElementList = this.range.getRangeParagraphElementList();
            console.log('选中的元素列表:', rangeElementList)
            if (!rangeElementList || !rangeElementList.length) {
                console.log('未找到选区内的段落元素');
                return;
            }
            ...
            for (const id of Ids) {
                ...
                if (paragraphElements.length > 0) {
                    ...
                    // 2. 设置段落标记的基本缩进属性
                    paragraphMarker.indent = targetIndent;
                    ...
                    if (paragraphElements.length > 1) {
                        ...
                        if (!isParagraphType) {  // 检查第一个元素是否是空格元素
                            ...
                            // 计算需要的空格数量
                            const spaceCount = Math.ceil(targetIndent); // 1px 对应一个空格
                            ...
                            if (targetIndent > 0) {
                                if (isSpaceElement) {
                                    ...
                                    if (paragraphMarker.indent) {
                                        newSpaceElement.indent = paragraphMarker.indent;
                                    }

                                    // 找到段落标记在数组中的位置
                                    const markerIndex = allElementList.indexOf(paragraphMarker);
                                    if (markerIndex >= 0) {
                                        // 插入到段落标记后面
                                        allElementList.splice(markerIndex, 0, newSpaceElement);
                                    }
                                }
                            } else if (isSpaceElement) {
                                // 缩进值为0且存在空格元素，删除空格元素
                                console.log('删除现有空格元素');
                                const spaceIndex = allElementList.indexOf(secondElement);
                                if (spaceIndex >= 0) {
                            ...
                            const secondElement2 = paragraphElements[1]
                            const isSpaceElement = secondElement2 && /^\s+$/.test(secondElement2.value);
                            console.log(`检查第一个元素是否是空格元素: ${secondElement2.value}`);
                            // 计算需要的空格数量
                            const spaceCount = Math.ceil(targetIndent); // 1px 对应一个空格
                            const spaceString = ' '.repeat(spaceCount);
                            ...
                            if (targetIndent > 0) {
                                if (isSpaceElement) {
                                    // 已有空格元素，调整其值
                                    console.log(`修改现有空格元素，设置为 ${spaceCount} 个空格`);
                                    secondElement2.value = spaceString;
                                } else {
                                    // 没有空格元素，创建一个新的
                                    console.log(`创建新的空格元素，包含 ${spaceCount} 个空格`);
                                    const IdString = String(id); // 强制转换为字符串
                                    ...
                                    if (markerIndex >= 0) {
                                        // 插入到段落标记后面
                                        allElementList.splice(markerIndex + 1, 0, newSpaceElement);
                                    }
                                }
                            } else if (isSpaceElement) {
                                // 缩进值为0且存在空格元素，删除空格元素
                                console.log('删除现有空格元素');
                                const spaceIndex = allElementList.indexOf(secondElement2);
                                if (spaceIndex >= 0) {
                                    allElementList.splice(spaceIndex, 1);
                                }
                            }
            ...
            console.error('设置缩进时出错:', error);
        } finally {
            // 强制重新计算和渲染，同时恢复选区
            console.log('准备重新渲染...');
            this.draw.render({
                isCompute: true,
                isSetCursor: false, // 不设置光标
                isSubmitHistory: true
            });
    ...
    /**
     * 设置文档第一个段落的缩进值
     * @param value 要设置的缩进值(0-100)
     */
    public setFirstParagraphIndent(value: number) {
        // 限制缩进值在0-1000之间
        const targetIndent = Math.max(0, Math.min(value, 1000));
        ...
        // 查找第一个段落元素
        const firstParagraphIndex = mainList.findIndex(el => el.type === ElementType.PARAGRAPH || el.type === ElementType.TITLE);

        // 如果没有找到段落元素，则直接设置第一个元素
        const targetIndex = firstParagraphIndex !== -1 ? firstParagraphIndex : 0;
        const firstElement = mainList[targetIndex];

        if (firstElement) {
            firstElement.indent = targetIndent;
            console.log('设置第一个段落缩进:', targetIndent, firstElement);
    ...
    public insertElementList(
        ...
        // 确保默认启用ensureWrapInParagraph选项
        const finalOptions: IInsertElementListOption = {
            ...options,
            ensureWrapInParagraph: options?.ensureWrapInParagraph !== false
        }
        this.draw.insertElementList(cloneElementList, finalOptions)
    }
    ...
    public appendElementList(
        ...
        if (isReadonly) return
        // 确保默认启用ensureWrapInParagraph选项
        const finalOptions: IAppendElementListOption = {
            ...options,
            ensureWrapInParagraph: options?.ensureWrapInParagraph !== false
        }
        this.draw.appendElementList(deepClone(elementList), finalOptions)
    }
    ...
    public setValue(payload: Partial<IEditorData>, options?: ISetValueOption) {
        ...
        console.log("this.draw.getElementList():", this.draw.getElementList())
