...
export class CommandAdapt {
  ...
  // 修复文档中的缩进值，确保所有缩进值在合理范围内(0-100)
  private fixIndentValues(elementList: IElement[]) {
    if (!elementList?.length) return;
    ...
    for (let i = 0; i < elementList.length; i++) {
      ...
      if (element.indent !== undefined) {
        element.indent = Math.max(0, Math.min(element.indent, 10000));
      }
      ...
      if (element.valueList?.length) {
        this.fixIndentValues(element.valueList);
      }
    }
  }
  ...
  // 修复当前文档中所有的缩进值
  public fixAllIndentValues() {
    // 修复主文档
    this.fixIndentValues(this.draw.getOriginalMainElementList());
    
    // 修复页眉
    this.fixIndentValues(this.draw.getHeaderElementList());
    
    // 修复页脚
    this.fixIndentValues(this.draw.getFooterElementList());
